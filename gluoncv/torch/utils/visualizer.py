...
__all__ = ["ColorMode", "VisImage", "Visualizer", "visualize_hm",
           "visualize_hm_xy", "visualize_pseudo_bbox", "visualize_kpt_offset"]
...
class GenericMask:
    ...
    def __init__(self, mask_or_polygons, height, width):
        try:
            ...
            raise ImportError('Unable to import `pycocotools`, please install it to process masks.')
...
def visualize_hm(all_hms, all_hms_weight, images):
    import cv2
    num_kpts = 17
    pixel_std = torch.Tensor([1.0, 1.0, 1.0]).view(-1, 1, 1)
    pixel_mean = torch.Tensor([103.53, 116.28, 123.675]).view(-1, 1, 1)
    for i in range(all_hms.shape[0]):
        img = images.tensor[i].cpu() * pixel_std + pixel_mean
        img = img.permute(1, 2, 0).numpy()
        cv2.imwrite('tmp_vis/ori_img.png', img)
        img = cv2.resize(img, (int(img.shape[1] / 8), int(img.shape[0] / 8)))
        for k in range(num_kpts):
            hm = all_hms[i, k].cpu().numpy() * 255
            hm = hm[:, :, None].repeat(3, axis=2)
            # hm_weight = all_hms_weight[i, k]
            hm_overlay = cv2.addWeighted(img, 0.5, hm, 1.0, 0)
            # hm_weight_overlay = cv2.addWeighted(hm_weight_overlay, 0.5, hm, 1.0, 0)
            cv2.imwrite('tmp_vis/heamtap_{:02d}.png'.format(k), hm_overlay)

def visualize_pseudo_bbox(bboxes, ori_bboxes, image):
    import cv2
    pixel_std = torch.Tensor([1.0, 1.0, 1.0]).view(-1, 1, 1)
    pixel_mean = torch.Tensor([103.53, 116.28, 123.675]).view(-1, 1, 1)
    img = image.cpu() * pixel_std + pixel_mean
    img = img.permute(1, 2, 0).numpy()
    cv2.imwrite('ori_img.png', img)
    img = cv2.imread('ori_img.png')
    for i in range(len(bboxes)):
        bbox = bboxes[i]
        ori_bbox = ori_bboxes[i]
        img = cv2.rectangle(img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (255, 0, 0), 1)
        img = cv2.rectangle(img, (int(ori_bbox[0]), int(ori_bbox[1])), (int(ori_bbox[2]), int(ori_bbox[3])), (0, 255, 0), 1)
    cv2.imwrite('tmp_vis/pseudo_bbox_img.png', img)

def visualize_hm_xy(hm_xs, hm_ys, hm_score, images, thresh=0.1):
    import cv2
    num_kpts = 17
    pixel_std = torch.Tensor([1.0, 1.0, 1.0]).view(-1, 1, 1)
    pixel_mean = torch.Tensor([103.53, 116.28, 123.675]).view(-1, 1, 1)
    for i in range(len(images)):
        ...
        img = img.permute(1, 2, 0).numpy()
        cv2.imwrite('tmp_vis/ori_img.png', img)
        img = cv2.imread('tmp_vis/ori_img.png')
        h, w = images.image_sizes[i]
        for k in range(num_kpts):
            for (xs, ys, score) in zip(hm_xs[i, k], hm_ys[i, k], hm_score[i, k]):
                if score > thresh and xs >=0 and xs < w and ys >=0 and ys < h:
                    hm = cv2.circle(img, (int(xs), int(ys)), 2, (255, 0, 0), 2)
                    cv2.imwrite('tmp_vis/heamtap_{:02d}.png'.format(k), hm)

def visualize_kpt_offset(images, boxlists, vis_dir='', thresh=0.5, cnt=0):
    import cv2
    ...
    for i in range(len(images)):
        img = images.tensor[i].cpu() * pixel_std + pixel_mean
        img = img.permute(1, 2, 0).numpy()
        cv2.imwrite('/workplace/data/motion_efs/home/manchenw/AdelaiDet/visualization/ori_img.png', img)
        img = cv2.imread('/workplace/data/motion_efs/home/manchenw/AdelaiDet/visualization/ori_img.png')
        if boxlists[i].scores.shape[0] == 0 or boxlists[i].scores[0] < thresh:
            cv2.imwrite(os.path.join(vis_dir, 'result_{:06d}.png'.format(cnt)), img)
        else:
            for j in range(len(boxlists[i])):
                if boxlists[i].scores[j] >= thresh:
                    color = np.random.randint(0, 256, (1, 3), dtype=np.uint8).tolist()[0]
                    img = draw_skeleton(img, boxlists[i].pred_keypoints[j].cpu().numpy(), color=color)
                    bbox = boxlists[i].pred_boxes[j].tensor[0].cpu().numpy().astype(np.int32)
                    img = cv2.rectangle(img, tuple(bbox[0:2]), tuple(bbox[2:4]), color, 2)
                    cv2.imwrite(os.path.join(vis_dir, 'result_{:06d}.png'.format(cnt)), img)
                else:
                    break
