...
def transform_test(imgs, short, max_size=1024, mean=(0.485, 0.456, 0.406),
    ...
    for img in imgs:
        ...
        img = mx.nd.image.to_tensor(img.as_nd_ndarray()).as_np_ndarray()
        img = mx.nd.image.normalize(img.as_nd_ndarray(), mean=mean, std=std)
        tensors.append(img.expand_dims(0).as_np_ndarray())
