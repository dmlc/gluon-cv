...
class SimplePoseDefaultTrainTransform(object):
    ...
    """
    def __init__(self, num_joints, joint_pairs, image_size, heatmap_size, sigma=2,
                 mean=(0.485, 0.456, 0.406),
                 std=(0.229, 0.224, 0.225), random_flip=True, scale_factor=0.25, rotation_factor=30,
                 **kwargs):
    ...
    def __call__(self, src, label):
        ...
        trans = get_affine_transform(center, s, r, self._image_size)
        w, h = self._image_size
        image = cv2.warpAffine(src, trans, (int(h), int(w)), flags=cv2.INTER_LINEAR)
