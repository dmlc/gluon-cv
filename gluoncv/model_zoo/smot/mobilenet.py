...
def _add_conv(out, channels=1, kernel=1, stride=1, pad=0,
              num_group=1, active=True, relu6=False, mish=False, norm_layer=BatchNorm, norm_kwargs=None):
...
def _add_conv_dw(out, dw_channels, channels, stride, relu6=False, mish=False,
                 norm_layer=BatchNorm, norm_kwargs=None):
    _add_conv(out, channels=dw_channels, kernel=3, stride=stride,
              pad=1, num_group=dw_channels, relu6=relu6, mish=mish,
              norm_layer=norm_layer, norm_kwargs=norm_kwargs)
    _add_conv(out, channels=channels, relu6=relu6, mish=mish,
              norm_layer=norm_layer, norm_kwargs=norm_kwargs)
...
class MobileNet(HybridBlock):
    ...
    def __init__(self, multiplier=1.0, classes=1000,
                 norm_layer=BatchNorm, norm_kwargs=None, mish=False, **kwargs):
        super(MobileNet, self).__init__(**kwargs)
        with self.name_scope():
            self.features = nn.HybridSequential(prefix='')
            with self.features.name_scope():
                _add_conv(self.features, channels=int(32 * multiplier), kernel=3, pad=1, stride=2,
                          norm_layer=norm_layer, norm_kwargs=norm_kwargs, mish=mish)
        ...
        with self.name_scope():
            ...
            with self.features.name_scope():
                _add_conv(self.features, int(32 * multiplier), kernel=3,
                          stride=2, pad=1, relu6=True, norm_layer=BatchNorm, norm_kwargs=norm_kwargs)
                ...
                _add_conv(self.features,
                          last_channels,
                          relu6=True,
                          norm_layer=BatchNorm, norm_kwargs=norm_kwargs)
...
def get_mobilenet(multiplier, pretrained=False, ctx=cpu(),
                  root='~/.mxnet/models', norm_layer=BatchNorm, norm_kwargs=None, mish=False, **kwargs):
    r"""MobileNet model from the
    `"MobileNets: Efficient Convolutional Neural Networks for Mobile Vision Applications"
    <https://arxiv.org/abs/1704.04861>`_ paper.
    ...
    norm_kwargs : dict
        ...
        for :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    net = MobileNet(multiplier, norm_layer=norm_layer, norm_kwargs=norm_kwargs, mish=mish, **kwargs)
    if pretrained:
