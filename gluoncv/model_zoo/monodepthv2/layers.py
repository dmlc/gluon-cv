...
def compute_depth_errors(gt, pred):
    ...
    """
    thresh = mx.nd.max((gt / pred), (pred / gt))
    a1 = (thresh < 1.25     ).float().mean()
    a2 = (thresh < 1.25 ** 2).float().mean()
    a3 = (thresh < 1.25 ** 3).float().mean()

    rmse = (gt - pred) ** 2
    rmse = mx.nd.sqrt(rmse.mean())

    rmse_log = (mx.nd.log(gt) - mx.nd.log(pred)) ** 2
    rmse_log = mx.nd.sqrt(rmse_log.mean())

    abs_rel = mx.nd.mean(mx.nd.abs(gt - pred) / gt)

    sq_rel = mx.nd.mean((gt - pred) ** 2 / gt)
