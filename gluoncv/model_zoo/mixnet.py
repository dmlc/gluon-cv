...
class MDConv(HybridBlock):
    ...
    def __init__(self, channels, kernel_size, stride, **kwargs):
        ...
        self.mixed_depthwise_conv = nn.HybridSequential()
        with self.mixed_depthwise_conv.name_scope():
             for i in range(self.num_groups):
                  self.mixed_depthwise_conv.add(nn.Conv2D(
                    channels=self.split_channels[i], 
    ...
    def hybrid_forward(self, F, x):
        ...
        x = [conv(t) for conv, t in zip(self.mixed_depthwise_conv, x_splits)]
        x = F.concat(*x, dim=1)
        return x
