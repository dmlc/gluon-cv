...
__all__ = ['FasterRCNN', 'get_faster_rcnn',
           ...
           'faster_rcnn_resnet50_v1d_custom',
...
def faster_rcnn_resnet50_v1d_voc(pretrained=False, pretrained_base=True, **kwargs):
    ...
    classes = VOCDetection.CLASSES
    pretrained_base = False if pretrained else pretrained_base
    base_network = resnet50_v1d(pretrained=pretrained_base, dilated=False,
                                use_global_stats=True, **kwargs)
    features = nn.HybridSequential()
    top_features = nn.HybridSequential()
    for layer in ['conv1', 'bn1', 'relu', 'maxpool', 'layer1', 'layer2', 'layer3']:
...
def faster_rcnn_resnet50_v1d_custom(classes, transfer=None, pretrained_base=True,
                                    pretrained=False, **kwargs):
    r"""Faster RCNN model with resnet50_v1d base network on custom dataset.
    ...
    if transfer is None:
        ...
        train_patterns = '|'.join(['.*dense', '.*rpn', '.*down(2|3|4)_conv',
                                   '.*layers(2|3|4)_conv'])
        return get_faster_rcnn(
            name='resnet50_v1d', dataset='custom', pretrained=pretrained,
            features=features, top_features=top_features, classes=classes,
            short=600, max_size=1000, train_patterns=train_patterns,
            nms_thresh=0.3, nms_topk=400, post_nms=100,
            roi_mode='align', roi_size=(14, 14), stride=16, clip=None,
        ...
        from ...model_zoo import get_model
        net = get_model('faster_rcnn_resnet50_v1d_' + str(transfer), pretrained=True, **kwargs)
        net.reset_class(classes)
    return net
