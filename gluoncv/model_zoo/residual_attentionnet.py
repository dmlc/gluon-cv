...
class UpsamplingBilinear2d(HybridBlock):
    ...
    def forward(self, x):
        return nd.contrib.BilinearResize2D(x.as_nd_ndarray(), height=self.size, width=self.size).as_np_ndarray()
...
class ResidualBlock(HybridBlock):
    ...
    def forward(self, x):
        residual = x
        out = self.bn1(x)
        out1 = npx.activation(out, act_type='relu')
        out = self.conv1(out1)
        out = self.bn2(out)
        out = npx.activation(out, act_type='relu')
        out = self.conv2(out)
        out = self.bn3(out)
        out = npx.activation(out, act_type='relu')
        out = self.conv3(out)
...
class AttentionModule_stage1(nn.HybridBlock):
    ...
    def forward(self, x):
        ...
        out = np.ones_like(out_softmax6) + out_softmax6
        out = out * out_trunk
...
class AttentionModule_stage2(nn.HybridBlock):
    ...
    def forward(self, x):
        ...
        out = np.ones_like(out_softmax4) + out_softmax4
        out = out * out_trunk
        out_last = self.last_blocks(out)
        return out_last
...
class AttentionModule_stage3(nn.HybridBlock):
    ...
    def forward(self, x):
        ...
        out = np.ones_like(out_softmax2) + out_softmax2
        out = out * out_trunk
        out_last = self.last_blocks(out)
        return out_last
