...
class COCOInstanceMaskScoreMetric(mx.metric.EvalMetric):
    ...
    def __init__(self, dataset, save_prefix, use_time=True, cleanup=False, score_thresh=1e-3):
        super(COCOInstanceMaskScoreMetric, self).__init__('COCOInstance')
        self.dataset = dataset
        self._img_ids = sorted(dataset.coco.getImgIds())
        self._current_id = 0
        self._cleanup = cleanup
        self._results_bbox = []
        self._results_maskscore = []
    ...
    def __del__(self):
        if self._cleanup:
            try:
                os.remove(self._filename_bbox)
                os.remove(self._filename_maskscore)
    ...
    def _dump_json(self):
        ...
        import json
        try:
            with open(self._filename_bbox, 'w') as f:
                json.dump(self._results_bbox, f)

            with open(self._filename_maskscore, 'w') as f:
                json.dump(self._results_maskscore, f)
        except IOError as e:
    ...
    def get(self):
        ...
        bbox_new_names, bbox_new_values = self._update('bbox_new_score')
    ...
    def update(self, pred_bboxes, pred_labels, pred_scores, pred_masks, pred_mask_scores, *args, **kwargs):
        ...
        pred_mask_score = pred_mask_score.flat[valid_pred].astype('float32')
