...
from ..estimators import CenterNetEstimator, YoloEstimator, SSDEstimator
from ..estimators import FasterRCNNEstimator
...
class ObjectDetection(BaseTask):
    def __init__(self, config, estimator=None, logger=None):
        ...
        if estimator is None:
            estimator = [CenterNetEstimator, YoloEstimator, SSDEstimator, FasterRCNNEstimator]
        ...
        self._config.scheduler_options = {
            ...
            'resume': self._config.get('resume', False),
        ...
        if self._config.search_strategy == 'hyperband':
            self._config.scheduler_options.update({
                'searcher': 'random',
                'max_t': self._config.train.epochs,
                'grace_period': self._config.get('grace_period', self._config.train.epochs // 4)
                })
