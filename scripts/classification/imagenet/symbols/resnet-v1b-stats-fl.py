...
# Calculate the output shape for a given set of pooling parameters and input shape.
# This leverages the convolution shape routine, except that output_features == input_features.
def pooling_nchw_out_shape(nchw_inshape, kernel, stride=(1,1), pad=(0,0)):
    (n, c, h, w) = nchw_inshape
    return conv_nchw_out_shape(nchw_inshape, c, kernel, stride, pad)
...
def apply_conv_bn_fusion(name, conv_bn_fusion_type = 'mainloop'):
    '''
        conv_bn_fusion_type : 
            mainloop = bn(a)+relu+conv+bn(s)
               stats = conv+bn(s)
    '''
    supported_fusions = ['mainloop', 'stats']
    assert conv_bn_fusion_type in supported_fusions, '{} is not supported'.format(conv_bn_fusion_type)
    return True
...
def residual_unit_norm_conv(data, nchw_inshape, num_filter, stride, dim_match, name, bottle_neck=True,
    ...
    if bottle_neck:
        # 1st NormConvolution: [no Stats Apply] [no Relu] Convolution Stats-Gen
        ...
        elem_count = element_count(nchw_shape)

        if conv_bn_fusion_type == 'mainloop':
            # Second NormConvolution: Stats-Apply Relu Convolution Stats-Gen
            ...
            # Third NormConvolution: Stats-Apply Relu Convolution Stats-Gen
        ...
        elif conv_bn_fusion_type == 'stats':
            ...
            bn1 = scale_bias_add_relu(conv1, conv_layout, batchnorm_layout, None, bn1_equiv_scale, bn1_equiv_bias, None, None,
            ...
            assert False, "{} is not supported yet".foramt(conv_bn_fusion_type)
        ...
        if dim_match:
            ...
            elem_count = element_count(sc_nchw_shape)
        ...
        raise ValueError('model not prepared to use NormalizedConvolution when bottle_neck = {}'.format(bottle_neck))
...
def resnet(units, num_stages, filter_list, num_classes, image_shape, batch_size, bottle_neck=True, workspace=256, dtype='float32', memonger=False,
           ...
           cudnn_bn_off=False, bn_eps=2e-5, bn_mom=0.9, conv_algo=-1, conv_bn_fusion_type = 'stats',
           fuse_bn_relu=False, fuse_bn_add_relu=False, force_tensor_core=False, use_dali=True, norm_conv=True):
    ...
    nchw_shape = (batch_size, nchannel, height, width)
    ...
    # Simple switch to control whether NormalizedConvolution and Finalize ops are part of the model.
    if norm_conv:
        print('**** Using NormalizedConvolution. ****')
        res_unit = residual_unit_norm_conv
    else:
        print('**** Using conventional Convolution/Batchnorm (*not* NormalizedConvolution). ****')
        res_unit = residual_unit
    ...
    if height <= 32:            # such as cifar10
        ...
        nchw_shape = conv_nchw_out_shape(nchw_shape, num_filter=filter_list[0], kernel=(3, 3), stride=(1,1), pad=(1, 1))
        ...
        body = mx.sym.Convolution(data=data, num_filter=filter_list[0], kernel=(7, 7), stride=(2,2), pad=(3, 3),
                                  ...
                                  cudnn_algo_fwd=conv_algo, cudnn_algo_bwd_data=conv_algo, cudnn_algo_bwd_filter=conv_algo,
                                  cudnn_tensor_core_only=force_tensor_core)
        nchw_shape = conv_nchw_out_shape(nchw_shape, num_filter=filter_list[0], kernel=(7, 7), stride=(2,2), pad=(3, 3))
        body = batchnorm(data=body, io_layout=conv_layout, batchnorm_layout=batchnorm_layout,
        ...
        nchw_shape = pooling_nchw_out_shape(nchw_shape, kernel=(3, 3), stride=(2, 2), pad=(1, 1))
    ...
    nchw_shape = pooling_nchw_out_shape(nchw_shape, kernel=(7, 7))
...
def get_symbol(num_classes, num_layers, image_shape, conv_workspace=256, dtype='float32',
               ...
               conv_algo=-1, fuse_bn_relu=False, fuse_bn_add_relu=False, force_tensor_core=False, use_dali=True,
               conv_bn_fusion_type='stats', **kwargs):
    """
    Adapted from https://github.com/tornadomeet/ResNet/blob/master/symbol_resnet.py
    (Original author Wei Wu) by Antti-Pekka Hynninen
    Implementing the original resnet ILSVRC 2015 winning network from:
    ...
    return resnet(units             = units,
                  ...
                  conv_bn_fusion_type = conv_bn_fusion_type,
                  use_dali          = use_dali)
