...
def residual_unit_norm_conv(data, nchw_inshape, num_filter, stride, dim_match, name, bottle_neck=True,
    ...
    nchw_outshape : tuple
        ...
        This shape-tracking is no longer needed now that NormConvolution does not require the 'elem_count' parameter.
    """
    ...
    if bottle_neck:
        ...
        nchw_shape = conv_nchw_out_shape(nchw_shape, num_filter=int(num_filter*0.25), kernel=(1,1), stride=stride, pad=(0,0))
        ...
        (conv2, conv2_sum, conv2_sum_squares) = \
            mx.sym.NormConvolution(conv1, no_norm=False, in_sum=conv1_sum, in_sum_squares=conv1_sum_squares, act_type='relu',
                                   num_filter=int(num_filter*0.25), kernel=(3,3), stride=(1,1), pad=(1,1),
                                   eps=bn_eps, momentum=bn_mom, fix_gamma=False,
                                   name=name + '_conv2', layout=conv_layout)
        nchw_shape = conv_nchw_out_shape(nchw_shape, num_filter=int(num_filter*0.25), kernel=(3,3), stride=(1,1), pad=(1,1))

        # Third NormConvolution: Stats-Apply Relu Convolution [no Stats-Gen]
        ...
        raise ValueError('model not prepared to use NormConvolution when bottle_neck = {}'.format(bottle_neck))
...
def resnet(units, num_stages, filter_list, num_classes, image_shape, batch_size, bottle_neck=True, workspace=256, dtype='float32', memonger=False,
    ...
    # Simple switch to control whether NormConvolution and Finalize ops are part of the model.
    if norm_conv:
        print('**** Using NormConvolution. ****')
        res_unit = residual_unit_norm_conv
    else:
        print('**** Using conventional Convolution/Batchnorm (*not* NormConvolution). ****')
