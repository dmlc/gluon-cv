...
from autogluon.task.object_detection import ObjectDetection as task
from mxnet import gluon

from gluoncv.data.mscoco.detection import COCODetection
from gluoncv.data.pascal_voc.detection import CustomVOCDetection
from gluoncv.pipelines.tasks.object_detection import ObjectDetection
...
def _get_dataset(root='~/.mxnet/datasets/voc', index_file_name='trainval', name=None,
    ...
    if format == 'voc':
        ...
        return CustomVOCDetection(root=root, splits=splits, name=name, **kwargs)

    elif format == 'coco':
        logger.info(">>> create dataset(COCO format)")
        return COCODetection(*args, **kwargs)
    else:
...
if __name__ == '__main__':
    ...
    _, dataset_test = get_dataset(args)
    ...
    task = ObjectDetection(args)
    estimator = task.fit()
    test_map = estimator.evaluate(dataset_test)
