...
class SliceLike(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        self.assign(out_data[0], req[0], mx.nd.slice_axis(
            in_data[0], axis=self._axis, begin=self._begin, end=self._begin + from_shape[self._axis]))

    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        from_shape = in_data[1].shape
        self.assign(mx.nd.slice_axis(in_grad[0], axis=self._axis,
                    begin=self._begin, end=self._begin + from_shape[self._axis]),
                    req[0], out_grad[0])
